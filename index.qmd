...
In this website you can find in detailed how the research design of the paper presented in AGILE 2025 was carried out in [HeiGIT](https://heigit.org/) mostly in the [Geoinformation for Humanitarian Aid](https://heigit.org/geoinformation-for-humanitarian-aid/) department. The goal is to increase transparency,
reproducibility, and openness of computational GIScience research as stated in the [Reproducible Paper Guidelines](https://osf.io/numa5) from AGILE. For this purpose, the following files are available in the top right-hand corner of the web site:

<ul class="fa-ul" style="list-style-type: none;">
<li>`r icons::icon_style( icons::academicons("overleaf"), fill="#1FA67A", scale=1.5)` Paper official links to the pdf file of the review paper written in LaTeX using Overleaf, which is available on the github repository folder ["paper_latex"](https://github.com/rruiz-s/agile-gscience-2024-rs-flood/blob/gh-pages/code/paper/paper_latex/AGILE_porto_alegre_study_v2_full_paper_agile.zip). </li>
<li>`r  icons::icon_style(icons::fontawesome("r-project"), scale = 1.5, fill = "#246ABE")` The paper rmarkdown contains the code to reproduce the results written in the paper, which is fully available in the github repository directory ["paper_rmarkdown"](https://github.com/rruiz-s/agile-gscience-2024-rs-flood/blob/gh-pages/code/paper/paper_rmarkdown/rmarkdown_pdf_paper_v2.zip). The Github repository also contains the rmarkdown file after it has been rendered as a [PDF](https://github.com/rruiz-s/agile-gscience-2024-rs-flood/tree/gh-pages/data/quarto_script_rs_paper_v2.pdf). </li>
<li>`r  icons::icon_style(icons::fontawesome("github"), scale = 1.5)` The Github repository contains all the data for the creation of this website, the aim of which is to replicate the research methodology and the data produced in the process. </li>
<li>`r icons::icon_style( icons::academicons("zenodo"), fill="#1c4cab", scale=1.5)` Zenodo keeps a copy of the data needed to replicate the methodology and reproduce the results obtained, mainly the data folder in the github repository. </li>
</ul>
...
- [[GIST](https://bok.eo4geo.eu/GIST)] Geographic Information Science and Technology
  ...
  - [[GC4](https://bok.eo4geo.eu/GC4)] Open Science. We used open data such as OpenStreetMap, as well as develop open and reproducible methods.
...
While calculating the connectivity took 81 and 54 seconds in total, the computation of the lack of redundancy was the most expensive process requiring 52 minutes (3097 seconds). Another demanding process was exporting the ORS file from the docker to the R environment using the get_graph.R script. We included the computation times of the main processes to obtain the connectivity and lack of redundancy results. Providing data to support the research question 3 related to the lack of redundancy took 76% of the time required, while calculating the connectivity in a intracity (IC) and core metropolitan (CM) scale only took 1.32% and 2% respectively.
...
DT::datatable(data=computation_agile_dt,
              colnames= c("Lane","Activity","Description","time (s)", "time (%)"),
