...
A positive aspect of these set of operations in the methodology is to allow demanding analysis without expensive hardware (See computational environment), which indirectly means a lower consume of energy and environment impact. If a still larger area of interest is required, we carried out tests with duckdb and its extension spatial ^[[DuckDB spatial: https://duckdb.org/docs/stable/extensions/spatial/overview.html](https://duckdb.org/docs/stable/extensions/spatial/overview.html)] with sucess.
...
## This is added to fix errors that arise when the first code chunk is not in R.
1+1
```
...
## Subdivide the flood extent

The code chunk "[AoI-6] Dect flood extent" includes one query with several CTE to create the table _flooding_subdivided_porto_.
...
#| eval: false
#| echo: true
#| code-summary: "[AoI-6] Detect flood extent"
...
The reason for simplifying the geometry by removing the slivers polygons was that spatial operations such as st_difference, which are required to obtain the post-disaster network, are computationally expensive. Based on the Carto’s post, we removed the slivers polygons using the calculated area ^[[https://icarto.es/en/improve-performance-making-the-difference-between-two-layers-with-postgis/](https://icarto.es/en/improve-performance-making-the-difference-between-two-layers-with-postgis/)].
Another technique in PostGIS to remove
these slivers polygons is via dilation and erosion ^[[https://trac.osgeo.org/postgis/wiki/UsersWikiBufferCoast](https://trac.osgeo.org/postgis/wiki/UsersWikiBufferCoast)] ^[[https://cdn2.hubspot.net/hubfs/2283855/PostGIS%20Day%202019%20-%20Overview.pdf](https://cdn2.hubspot.net/hubfs/2283855/PostGIS%20Day%202019%20-%20Overview.pdf)], which could be used in future studies. The table _flooding_cleaned_porto_union_simple_ is a lighter version of the flood extent

```{sql}
#| eval: false
#| echo: true
#| code-summary: "[AoI-7] Simplifying geometry"
