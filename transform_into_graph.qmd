...
Firstly, in the _ors-config.yml_ from OpenRotueService (ORS), the “source file” parameter located in the line 102 is set to find the directory containing the OSM road network. In this case, we set the directory to _/home/ors/files/porto_alegre_urban_center.osm.pbf_. Secondly, before running the docker, we added the file "porto_alegre_urban_center.osm.pbf" obtained in the lane obtain routable network to the directory files inside the docker. 
...
![Fig](data/figures/openrouteservice.png)
...
![Fig.1 Length from the networks' components](data/figures/components_length_network.svg)
...
CREATE TABLE centrality_bidirect_temp_centrality_post_5 AS
SELECT t1.*,
    t2."bdrctid"
FROM
    temp_centrality_post_5 t1
JOIN
    net_largest_post_5 t2
ON
    t1.id = t2.id;
--- The final product requries  99127   
select max("bdrctid") from centrality_bidirect_temp_centrality_post_5 ;
create sequence bididirect_id_temp_centrality_post_5_seq start 99127;
update centrality_bidirect_temp_centrality_post_5 
set "bdrctid" = nextval('bididirect_id_temp_centrality_post_5_seq')
where "bdrctid" is null ;
---- verify
select count(*) 
from centrality_bidirect_temp_centrality_post_5
where "bdrctid" is null; --- 0
----
create table centrality_bidirect_temp_centrality_post_5_bidirect_group as 
select 
       "bdrctid",
...
---
---- now recover the id
create table centrality_bidirect_temp_centrality_post_5_bidirect_group_id as 
select t1.*, t2.id
from centrality_bidirect_temp_centrality_post_5_bidirect_group t1
join centrality_bidirect_temp_centrality_post_5 t2 
on t1."bdrctid" = t2."bdrctid"; 
---- add geometries
create table centrality_bidirect_temp_centrality_post_5_cleaned as
select t1.*,
    t2.geom,
    t2.toid,
    t2.fromid,
    ...
    t2."undrcti"
from 
    centrality_bidirect_temp_centrality_post_5_bidirect_group_id t1
join
    net_largest_post_5 t2
on
  t1.id = t2.id;

SELECT * FROM net_largest_post_5;
